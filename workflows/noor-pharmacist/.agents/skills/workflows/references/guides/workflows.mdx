...
sequence of commands. This is a fundamental requirement of Temporal's replay mechanism:
when a worker restarts or a workflow is recovered, Temporal re-executes the workflow code
...
When determinism enforcement is enabled, your workflow code runs inside Temporal's Python sandbox. The sandbox:
...
You can find more details about Temporal's sandbox [here](https://docs.temporal.io/develop/python/python-sdk-sandbox).
:::

When enabled, the SDK sets `sandboxed=True` on the underlying Temporal workflow definition.
No custom sandbox policy is applied, the standard Temporal sandbox behavior is used as-is.
...
class MyWorkflow:
    ...
    async def run(self, input: str) -> str:
        # This code runs WITHOUT the Temporal sandbox.
...
Both context managers map directly to their Temporal equivalents
(`temporalio.workflow.unsafe.imports_passed_through()` and
`temporalio.workflow.unsafe.sandbox_unrestricted()` respectively).
...
:::warning
**All workflow entrypoint parameters and return types must have type annotations.** An entrypoint missing any type hint on its arguments or return value will fail validation at registration time. Always provide explicit types for every parameter and the return type.
...
class BadWorkflow:
    @workflows.workflow.entrypoint
    async def run(self, data, count=1):
        pass
...
class GoodWorkflow:
    @workflows.workflow.entrypoint
    async def run(self, data: str, count: int = 1) -> dict:
        pass
```
:::
...
```python
await client.workflows.executions.reset_workflow_async(
...
- [Activity Guide](activities) - Learn how to implement work units
- [Observability Guide](observability) - Monitor and trace your workflows
- [Scheduling Guide](scheduling) - Learn about advanced scheduling options
